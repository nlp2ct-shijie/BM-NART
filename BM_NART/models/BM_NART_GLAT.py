...
@register_model("BM_NART_GLAT")
class BM_NART_GLAT(NAT_ctc_model):
        ...
        if glat is not None:
            with torch.no_grad():
                ...
                best_aligns = best_alignment(log_prob.float(), tgt_tokens, input_length,
                                             ...
                                             self.blank_idx,
                                             True)
                aligned_tgt_tokens = convert_alignment_to_symbol(best_aligns, tgt_tokens, self.blank_idx, decoder_padding_mask, self.pad)
                ...
                if not getattr(self.args, "fast_glat", False):
                    ...
                    input_mask = (torch.rand(prev_nat.shape, device=word_ins_out.device) < keep_prob).bool()
...
@register_model_architecture("BM_NART_GLAT", "BM_NART_GLAT")
...
@register_model_architecture("BM_NART_GLAT", "BM_NART_GLAT_BIG")
